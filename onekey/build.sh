...
echo
echo
echo "本脚本仅适用于在Ubuntu环境下编译 https://github.com/yhl452493373/OpenWrt_x86-r2s-r4s"
echo
echo
sleep 2s
sudo apt-get update
...
svn export https://github.com/yhl452493373/OpenWrt_x86-r2s-r4s/trunk/devices openwrt/devices
...
if [ -f "devices/common/default-settings" ]; then
	sed -i 's/192.168.2.1/$ip/' package/*/*/my-default-settings/files/etc/uci-defaults/95-default-settings
fi
